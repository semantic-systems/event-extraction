    ...
    def forward(self,
        ...
        if mode == "train":
            ...
            return SingleLabelClassificationForwardOutput(loss=loss.item(), prediction_logits=head_output.output,
                                                          encoded_features=encoded_feature.encoded_feature.detach().numpy())
...
class SingleLabelContrastiveSequenceClassification(SingleLabelSequenceClassification):
    ...
    def forward(self,
        ...
        if mode == "train":
            loss = self.loss(head_output.output, input_feature.labels)
            new_shape = (int(head_output.output.shape[0]/2), 2, head_output.output.shape[-1])
            contrastive_features = head_output.output.reshape(new_shape)
            contrastive_loss = self.contrastive_loss(contrastive_features, input_feature.labels[:int(head_output.output.shape[0]/2)])
