...
class LatexTableWriter(object):
    ...
    def get_result_df(result_instances: List[Result]) -> pd.DataFrame:
        ...
        data = {'seed': [result.seed for result in result_instances],
                ...
                'margin': [result.margin for result in result_instances],
                'coeff': [result.coeff for result in result_instances]
                }
...
class ConfigWriter(object):
    ...
    def change_field_of_all(dir: str):
        ...
        for file in files:
            ...
            config["loss"]["var_cov_loss"]["coef"] = 9
            ...
            output = config["model"]["output_path"]
            updated_output = output.replace("coef/1", "coef/9")
            config["model"]["output_path"] = updated_output
...
if __name__ == "__main__":
    # ConfigWriter.change_field_of_all("event_extractor/configs/tweeteval/final/cohort11/coef/9/")
